...
func (_q *ApiKeyQuery) QueryRole() *RoleQuery {
...
func (_q *ApiKeyQuery) WithRole(opts ...func(*RoleQuery)) *ApiKeyQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.ApiKey.Query().
//		GroupBy(apikey.FieldCreatedByID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ApiKey.Query().
//		Select(apikey.FieldCreatedByID).
//		Scan(ctx, &v)
...
func (_q *ApiKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ApiKey, error) {
	...
	if _q.withOwner != nil || _q.withRole != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withRole; query != nil {
		if err := _q.loadRole(ctx, query, nodes, nil,
			func(n *ApiKey, e *Role) { n.Edges.Role = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ApiKeyQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*ApiKey, init func(*ApiKey), assign func(*ApiKey, *Role)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*ApiKey)
	for i := range nodes {
		if nodes[i].api_key_role == nil {
			continue
		}
		fk := *nodes[i].api_key_role
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "api_key_role" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
