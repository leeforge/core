...
type DomainQuery struct {
	...
	withDomainType  *DomainTypeQuery
	withMemberships *DomainMembershipQuery
	withBindings    *ResourceDomainBindingQuery
	withFKs         bool
...
// QueryDomainType chains the current query on the "domain_type" edge.
func (_q *DomainQuery) QueryDomainType() *DomainTypeQuery {
	query := (&DomainTypeClient{config: _q.config}).Query()
...
func (_q *DomainQuery) QueryMemberships() *DomainMembershipQuery {
	query := (&DomainMembershipClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *DomainQuery) QueryBindings() *ResourceDomainBindingQuery {
	query := (&ResourceDomainBindingClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *DomainQuery) Clone() *DomainQuery {
	...
	return &DomainQuery{
		...
		withDomainType:  _q.withDomainType.Clone(),
...
// WithDomainType tells the query-builder to eager-load the nodes that are connected to
// the "domain_type" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *DomainQuery) WithDomainType(opts ...func(*DomainTypeQuery)) *DomainQuery {
	query := (&DomainTypeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDomainType = query
	return _q
}
...
func (_q *DomainQuery) WithMemberships(opts ...func(*DomainMembershipQuery)) *DomainQuery {
	query := (&DomainMembershipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DomainQuery) WithBindings(opts ...func(*ResourceDomainBindingQuery)) *DomainQuery {
	query := (&ResourceDomainBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Domain.Query().
//		GroupBy(domain.FieldCreatedByID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Domain.Query().
//		Select(domain.FieldCreatedByID).
//		Scan(ctx, &v)
...
func (_q *DomainQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Domain, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [3]bool{
			_q.withDomainType != nil,
			_q.withMemberships != nil,
			_q.withBindings != nil,
		}
	)
	if _q.withDomainType != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withDomainType; query != nil {
		if err := _q.loadDomainType(ctx, query, nodes, nil,
			func(n *Domain, e *DomainType) { n.Edges.DomainType = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *Domain) { n.Edges.Memberships = []*DomainMembership{} },
			func(n *Domain, e *DomainMembership) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withBindings; query != nil {
		if err := _q.loadBindings(ctx, query, nodes,
			func(n *Domain) { n.Edges.Bindings = []*ResourceDomainBinding{} },
			func(n *Domain, e *ResourceDomainBinding) { n.Edges.Bindings = append(n.Edges.Bindings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DomainQuery) loadDomainType(ctx context.Context, query *DomainTypeQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *DomainType)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Domain)
	for i := range nodes {
		if nodes[i].domain_type_domains == nil {
			continue
		}
		fk := *nodes[i].domain_type_domains
	...
	query.Where(domaintype.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "domain_type_domains" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *DomainQuery) loadMemberships(ctx context.Context, query *DomainMembershipQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *DomainMembership)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(domainmembership.FieldDomainID)
	}
	query.Where(predicate.DomainMembership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(domain.MembershipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *DomainQuery) loadBindings(ctx context.Context, query *ResourceDomainBindingQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *ResourceDomainBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Domain)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(resourcedomainbinding.FieldDomainID)
	}
	query.Where(predicate.ResourceDomainBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(domain.BindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
