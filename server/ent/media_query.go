...
func (_q *MediaQuery) QueryOwner() *UserQuery {
...
func (_q *MediaQuery) QueryFolder() *MediaFolderQuery {
...
func (_q *MediaQuery) QueryPosts() *PostQuery {
...
func (_q *MediaQuery) WithOwner(opts ...func(*UserQuery)) *MediaQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MediaQuery) WithFolder(opts ...func(*MediaFolderQuery)) *MediaQuery {
	query := (&MediaFolderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MediaQuery) WithPosts(opts ...func(*PostQuery)) *MediaQuery {
	query := (&PostClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Media.Query().
//		GroupBy(media.FieldCreatedByID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	...
	if query := _q.withOwner; query != nil {
		if err := _q.loadOwner(ctx, query, nodes, nil,
			func(n *Media, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFolder; query != nil {
		if err := _q.loadFolder(ctx, query, nodes, nil,
			func(n *Media, e *MediaFolder) { n.Edges.Folder = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPosts; query != nil {
		if err := _q.loadPosts(ctx, query, nodes,
			func(n *Media) { n.Edges.Posts = []*Post{} },
			func(n *Media, e *Post) { n.Edges.Posts = append(n.Edges.Posts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MediaQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Media, init func(*Media), assign func(*Media, *User)) error {
...
func (_q *MediaQuery) loadFolder(ctx context.Context, query *MediaFolderQuery, nodes []*Media, init func(*Media), assign func(*Media, *MediaFolder)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Media)
	for i := range nodes {
...
func (_q *MediaQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*Media, init func(*Media), assign func(*Media, *Post)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(media.PostsTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(media.PostsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(media.PostsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(media.PostsPrimaryKey[1]))
...
func (_q *MediaQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withOwner != nil {
			_spec.Node.AddColumnOnce(media.FieldOwnerID)
		}
		if _q.withFolder != nil {
			_spec.Node.AddColumnOnce(media.FieldFolderID)
		}
	}
