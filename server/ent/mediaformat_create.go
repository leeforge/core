...
func (_c *MediaFormatCreate) SetOwnerDomainID(v uuid.UUID) *MediaFormatCreate {
	_c.mutation.SetOwnerDomainID(v)
	return _c
}

// SetNillableOwnerDomainID sets the "owner_domain_id" field if the given value is not nil.
func (_c *MediaFormatCreate) SetNillableOwnerDomainID(v *uuid.UUID) *MediaFormatCreate {
	if v != nil {
...
func (_c *MediaFormatCreate) check() error {
	...
	if _, ok := _c.mutation.MediaID(); !ok {
		return &ValidationError{Name: "media_id", err: errors.New(`ent: missing required field "MediaFormat.media_id"`)}
	}
	if len(_c.mutation.MediaIDs()) == 0 {
		return &ValidationError{Name: "media", err: errors.New(`ent: missing required edge "MediaFormat.media"`)}
	}
	return nil
}
