...
//		GroupBy(passwordresettoken.FieldCreatedByID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.PasswordResetToken.Query().
//		Select(passwordresettoken.FieldCreatedByID).
//		Scan(ctx, &v)
...
func (_q *PasswordResetTokenQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*PasswordResetToken, init func(*PasswordResetToken), assign func(*PasswordResetToken, *User)) error {
	...
	for i := range nodes {
		if nodes[i].password_reset_token_user == nil {
			continue
		}
		fk := *nodes[i].password_reset_token_user
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "password_reset_token_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
