...
func (_q *PostQuery) QueryMedia() *MediaQuery {
...
func (_q *PostQuery) WithMedia(opts ...func(*MediaQuery)) *PostQuery {
	query := (&MediaClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := _q.withMedia; query != nil {
		if err := _q.loadMedia(ctx, query, nodes,
			func(n *Post) { n.Edges.Media = []*Media{} },
			func(n *Post, e *Media) { n.Edges.Media = append(n.Edges.Media, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PostQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*Post, init func(*Post), assign func(*Post, *Media)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Post)
	nids := make(map[uuid.UUID]map[*Post]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.MediaTable)
		s.Join(joinT).On(s.C(media.FieldID), joinT.C(post.MediaPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(post.MediaPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.MediaPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
