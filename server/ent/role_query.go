...
func (_q *RoleQuery) QueryAPIKeys() *ApiKeyQuery {
...
func (_q *RoleQuery) WithAPIKeys(opts ...func(*ApiKeyQuery)) *RoleQuery {
	query := (&ApiKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withAPIKeys; query != nil {
		if err := _q.loadAPIKeys(ctx, query, nodes,
			func(n *Role) { n.Edges.APIKeys = []*ApiKey{} },
			func(n *Role, e *ApiKey) { n.Edges.APIKeys = append(n.Edges.APIKeys, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *RoleQuery) loadAPIKeys(ctx context.Context, query *ApiKeyQuery, nodes []*Role, init func(*Role), assign func(*Role, *ApiKey)) error {
	...
	query.Where(predicate.ApiKey(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.APIKeysColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.role_api_keys
		if fk == nil {
			return fmt.Errorf(`foreign-key "role_api_keys" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "role_api_keys" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *RoleQuery) loadParent(ctx context.Context, query *RoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *Role)) error {
	...
	nodeids := make(map[uuid.UUID][]*Role)
	for i := range nodes {
		if nodes[i].role_children == nil {
			continue
		}
		fk := *nodes[i].role_children
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "role_children" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RoleQuery) loadChildren(ctx context.Context, query *RoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *Role)) error {
	...
	for _, n := range neighbors {
		fk := n.role_children
		if fk == nil {
			return fmt.Errorf(`foreign-key "role_children" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "role_children" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
