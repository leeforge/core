...
//
//	client.Tenant.Query().
//		GroupBy(tenant.FieldCreatedByID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Tenant.Query().
//		Select(tenant.FieldCreatedByID).
//		Scan(ctx, &v)
...
func (_q *TenantQuery) loadChildren(ctx context.Context, query *TenantQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Tenant)) error {
	...
	for _, n := range neighbors {
		fk := n.ParentTenantID
		if fk == nil {
			return fmt.Errorf(`foreign-key "parent_tenant_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
