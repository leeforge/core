...
func (tx *Tx) init() {
	...
	tx.APIPermissionMap = NewAPIPermissionMapClient(tx.config)
	...
	tx.DomainMembership = NewDomainMembershipClient(tx.config)
	tx.DomainRelation = NewDomainRelationClient(tx.config)
	tx.DomainType = NewDomainTypeClient(tx.config)
	tx.InvitationToken = NewInvitationTokenClient(tx.config)
	tx.Media = NewMediaClient(tx.config)
	tx.MediaFolder = NewMediaFolderClient(tx.config)
	...
	tx.ResourceDomainBinding = NewResourceDomainBindingClient(tx.config)
...
// applies a query, for example: APIPermission.QueryXXX(), the query will be executed
