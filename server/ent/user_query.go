...
type UserQuery struct {
	...
	withSystemErrors  *SystemErrorQuery
...
// QuerySystemErrors chains the current query on the "system_errors" edge.
func (_q *UserQuery) QuerySystemErrors() *SystemErrorQuery {
	query := (&SystemErrorClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSystemErrors:  _q.withSystemErrors.Clone(),
...
// WithSystemErrors tells the query-builder to eager-load the nodes that are connected to
// the "system_errors" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSystemErrors(opts ...func(*SystemErrorQuery)) *UserQuery {
	query := (&SystemErrorClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSystemErrors = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withSystemErrors; query != nil {
		if err := _q.loadSystemErrors(ctx, query, nodes,
			func(n *User) { n.Edges.SystemErrors = []*SystemError{} },
			func(n *User, e *SystemError) { n.Edges.SystemErrors = append(n.Edges.SystemErrors, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*User, init func(*User), assign func(*User, *Media)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(media.FieldOwnerID)
	}
...
func (_q *UserQuery) loadSystemErrors(ctx context.Context, query *SystemErrorQuery, nodes []*User, init func(*User), assign func(*User, *SystemError)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(systemerror.FieldUserID)
	}
	query.Where(predicate.SystemError(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SystemErrorsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
