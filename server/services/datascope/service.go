...
func (s *Service) resolveRoleDefaultScope(
	...
	domainRoles, err := s.db.Role.Query().
		Where(
			role.OwnerDomainIDEQ(domainID),
			role.CodeIn(roleCodes...),
		).
		All(ctx)
	if err != nil || len(domainRoles) == 0 {
		return nil, false, nil
	}

	maxBaseScope := ScopeSelf
	pluginScopes := make([]ScopeType, 0, len(domainRoles))
	seenPluginScope := make(map[ScopeType]struct{}, len(domainRoles))

	for _, r := range domainRoles {
		scope := ScopeType(strings.ToUpper(strings.TrimSpace(r.DefaultDataScope)))
		if scope == "" {
			continue
		}
		if isBaseScopeType(scope) {
